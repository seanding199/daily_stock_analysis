...
class BOLLIndicator:
    ...
    def calculate(self, df: pd.DataFrame) -> BOLLResult:
        """
        计算布林带指标
    ...
    def _analyze_status(
        ...
        # 2. 判断是否突破上轨
        if price > upper:
            signal = f"突破上轨 {dist_upper:.1f}%，强势特征，但需防止回落"
            return BOLLStatus.BREAK_UPPER, signal, "高"
        
        # 3. 判断是否跌破下轨
        if price < lower:
            signal = f"跌破下轨 {dist_lower:.1f}%，弱势特征，等待企稳信号"
...
def example_usage():
    ...
    # 1. 准备数据（示例数据）
    dates = pd.date_range('2025-12-01', periods=60, freq='D')
    prices = [10 + i*0.1 + np.random.randn()*0.5 for i in range(60)]
    df = pd.DataFrame({
        'date': dates,
        'close': prices
    })
    ...
    print(f"\n价格位置: {result.position_pct:.1f}%")
    print(f"带宽: {result.bandwidth:.2f}%")
    print(f"距上轨: {result.distance_to_upper:.2f}%")
    print(f"距下轨: {result.distance_to_lower:.2f}%")
    print(f"\n状态: {result.status.value}")
    print(f"信号: {result.signal}")
    print(f"风险等级: {result.risk_level}")
    
    # 5. 获取支撑/压力位
    levels = boll.get_support_resistance(df)
    print(f"\n支撑位: {levels['support']}")
