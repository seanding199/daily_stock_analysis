...
class Config:
    ...
    # === 推荐选股配置 ===
    scan_stock_list: List[str] = field(default_factory=list)   # 扫描池股票代码
    scan_boards: List[str] = field(default_factory=list)       # 扫描板块名称
    scan_min_score: int = 65                                    # 最低推荐评分
    scan_top_n: int = 10                                        # 推荐数量
    scan_mode: str = 'fast'                                     # fast=仅技术面 / deep=+AI
    scan_max_workers: int = 5                                   # 扫描并发数
    ...
    openai_temperature: float = 0.7  # 温度参数（0.0-2.0，默认0.7）

    # AI 请求配置（防止 429 限流）
    ai_request_delay: float = 2.0  # 请求间隔（秒）
    ai_max_retries: int = 5  # 最大重试次数
    ai_retry_delay: float = 5.0  # 重试基础延时（秒）
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            ai_request_delay=float(os.getenv('AI_REQUEST_DELAY', os.getenv('GEMINI_REQUEST_DELAY', '2.0'))),
            ai_max_retries=int(os.getenv('AI_MAX_RETRIES', os.getenv('GEMINI_MAX_RETRIES', '5'))),
            ai_retry_delay=float(os.getenv('AI_RETRY_DELAY', os.getenv('GEMINI_RETRY_DELAY', '5.0'))),
